...
fn test_e2e_with_config() {
    ...
    HttpRateSvcProcessor::build_rate_layer(cfg).expect("e2e with_config build must succeed");
}
...
fn test_e2e_build() {
    ...
    let layer = HttpRateSvcProcessor::build_rate_layer(cfg).expect("e2e build must succeed");
