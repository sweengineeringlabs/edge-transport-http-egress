...
fn cache_struct_dep_reqwest_layer_attaches_to_middleware_client_int_test() {
    ...
    let layer =
        HttpCacheSvcProcessor::build_cache_layer(cfg).expect("build_cache_layer must succeed");
    ...
    assert_middleware::<edge_transport_http_egress_cache::MiddlewareHttpCache>();
